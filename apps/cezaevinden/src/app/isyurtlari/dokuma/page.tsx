...
export default function DokumaPage() {
  const [products, setProducts] = useState<Product[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetch('/api/isyurtlari/products?category=dokuma')
      .then((res) => res.json())
      .then((data) => {
        setProducts(data);
        setLoading(false);
  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <h1 className="text-3xl font-bold mt-2">Dokuma Ürünleri</h1>
        <p className="text-gray-600 mt-2">Adalet Bakanlığı İsyurtları tarafından üretilen dokuma ürünleri</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {products.map((product) => (
          ...
          >
            <div className="h-48 bg-gradient-to-br from-indigo-100 to-indigo-200 rounded-t-lg overflow-hidden flex items-center justify-center">
              {product.imageUrl ? (
                <img
                ...
                <div className="text-center">
                  ...
                  <p className="text-indigo-600 text-xs mt-2 px-2">{product.name}</p>
                </div>
              )}
            </div>
            <div className="p-4">
